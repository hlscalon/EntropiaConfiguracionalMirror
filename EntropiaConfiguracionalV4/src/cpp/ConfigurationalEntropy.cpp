...
const std::tuple<int, Vector<int>> ConfigurationalEntropy::generate_subgraphs(int m, int n) {
	std::map<Vector<int>, Graph> canonicalLabels;
	...
	for (int i = 0; i < m; ++i) {
		...
		if (found) {
			...
			if (!graph.is_connected()) {
				...
				canonicalLabels[nearestNeighbors] = std::move(graph);
			}
		} else {
...
void ConfigurationalEntropy::check_isomorfism(Vector<Graph> & graphsIso, Graph & graphIsoTmp, int & isoLabel, Vector<int> & labelTotal, int totalNodes) {
	UndirectedGraph clgraph = *graphIsoTmp.get_ugraph();
	int idx = find_isomorphic_index(graphsIso, clgraph);
